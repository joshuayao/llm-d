...
| --- | --- | --- | --- |
| Intel XPU | `modelserver/xpu/vllm/` | Qwen/Qwen3-4B | Update router `modelName` if you change this model |
...
Create the `llm-d-hf-token` secret in the namespace. The router reads `HF_TOKEN` to reach gated tokenizers — Qwen/Qwen3-4B is public but the secret makes swapping in a gated model a no-op. See [helpers/hf-token.md](../../helpers/hf-token.md) for the full helper.
...
Apply the Intel XPU vLLM Kustomize overlay:
